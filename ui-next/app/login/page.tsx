import Image from "next/image";
import { redirect } from "next/navigation";
import { LoginForm } from "@/components/LoginForm";
import { getMe } from "@/lib/api";
import { landingFor } from "@/lib/session";

export const dynamic = "force-dynamic";

export default async function LoginPage() {
  // Already signed in? Don't show the form again — send them somewhere useful.
  try {
    const me = await getMe();
    redirect(me.must_change_password ? "/change-password" : landingFor(me));
  } catch {
    /* not signed in — fall through to the form */
  }

  return (
    <div className="min-h-screen w-full bg-[#F8F9FB] flex items-center justify-center px-4">
      <div className="w-full max-w-[380px]">
        <div className="flex items-center gap-2.5 mb-7">
          <Image
            src="/rayna-logo.png"
            alt=""
            width={116}
            height={34}
            className="h-[34px] w-auto"
            priority
          />
          <span className="text-[10.5px] font-semibold uppercase tracking-[0.14em] text-[#98A2B3] border-l border-[#E4E7EC] pl-2.5">
            Price Intelligence
          </span>
        </div>

        <div className="bg-white border border-[#E4E7EC] rounded-[14px] px-6 py-6 shadow-[0_1px_3px_rgba(16,24,40,0.06)]">
          <h1 className="text-[18px] font-bold text-[#101828] tracking-[-0.01em]">
            Sign in
          </h1>
          <p className="text-[12.5px] text-[#667085] mt-1 mb-5">
            Use the account your administrator set up for you.
          </p>
          <LoginForm />
        </div>

        <p className="text-[11.5px] text-[#98A2B3] text-center mt-5 leading-relaxed">
          Need access, or forgotten your password?
          <br />
          Ask an administrator to set a new one.
        </p>
      </div>
    </div>
  );
}
